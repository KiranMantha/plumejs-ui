...
const astTransformer: ts.TransformerFactory<ts.SourceFile> = (context: ts.TransformationContext) => {
  return (sourceFile) => {
    const visitor: ts.Visitor = (node: ts.Node): ts.Node | ts.Node[] => {
      if (
        ts.isDecorator(node) &&
        containDecorators([DecoratorTypes.COMPONENT, DecoratorTypes.INJECTABLE, DecoratorTypes.INPUT], node)
      ) {
        return undefined;
      }
      ...
      if (ts.isPropertyDeclaration(node)) {
        if (node.decorators) {
          ...
          if (inputDecorator) {
            ...
            const updatedPropertyNode = factory.updatePropertyDeclaration(
              node,
              undefined,
              node.modifiers,
              node.name,
